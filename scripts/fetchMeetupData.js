...
async function getNextDataJsonFromPage(url) {
  ...
  const regex = /<script\s+id="__NEXT_DATA__"\s*type="application\/json">\s*(.*?)(?=<\/script>|\s*<\/body>)/s;
  const match = htmlContent.match(regex);

  const data = match[0].replace('<script id="__NEXT_DATA__" type="application/json">', '').replace('</script>', '');
...
async function fetchMeetupEvents(url) {
  ...
  for (const key of Object.keys(state)) {
    if (key.includes('Event')) {
      const { id, title, eventUrl, dateTime, going, venue, rsvpSettings } = state[key];
      ...
      // get venue details
      const { name, address, city, state: province, country } = state[venue.__ref];
...
async function main() {
  try {
    ...
    const upcoming = await fetchMeetupEvents(`${MEETUP_URL}/events/?type=upcoming`);
